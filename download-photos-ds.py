...
def download_visit_photos(attTable, photoFeatureClass, repeatFeatureClass, visitFeatureClass, dataPhotoLocation, originalsLocation, photoTypeField):
    ...
    for item in att_cursor:
        ...
        for row in photo_cursor: # there is actually just one row
            ...
            repeat_cursor = da.SearchCursor(repeatFeatureClass, field_names = ['parentglobalid',photoTypeField], where_clause = "GLOBALID = " +"'" + fk_repeat + "'")
            # now go use the parent globaliD to get to the visit record
            for row in repeat_cursor:
                fk_visit = str(row[0])
                print(fk_visit)
                photo_type = repeat_cursor[1]
